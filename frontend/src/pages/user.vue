<template>
  <GameShell role-label="隊伍工作區" identity="第 7 隊・鳳凰社" section="我的隊伍" kicker="TEAM WORKSPACE" title="鳳凰社，下一步怎麼走？" subtitle="在市場之間移動、觀察行情，讓每一枚金幣都替你靠近終點。" :nav-items="navItems" :connected="false" demo :period="2" :elapsed-ms="1260000" status="running" :money="218" @sign-out="goLogin">
    ...
    <div class="two-column"><section class="section-block"><div class="section-block__head"><div><h2>現在可以做什麼</h2><p>互動前會再次確認現場條件</p></div></div><div class="action-grid"><RouterLink v-for="action in actions" :key="action.label" :to="action.to" class="action-tile"><span class="action-tile__icon"><Icon :name="action.icon" size="md" /></span><strong>{{ action.label }}</strong><span>{{ action.detail }}</span></RouterLink></div></section><section class="section-block"><div class="section-block__head"><div><h2>隊伍紀錄</h2><p>最近的金錢與物資變化</p></div><button class="text-button" type="button">全部紀錄 →</button></div><div class="event-list"><div v-for="event in events" :key="event.title" class="event-item"><span class="event-icon"><Icon :name="event.icon" size="sm" /></span><div><strong>{{ event.title }}</strong><span>{{ event.detail }}</span></div><span class="event-time">{{ event.time }}</span></div></div></section></div>
  </GameShell>
</template>
...
const actions = [{ to: '/user/market', icon: 'market', label: '前往市場交易', detail: '查看八個市場的當期行情' }, { to: '/user/challenges', icon: 'spark', label: '挑戰隱藏魔王', detail: '答題成功可獲得金幣' }, { to: '/user/map', icon: 'map', label: '挑戰據點', detail: '第 3 時段起開放佔領' }]
